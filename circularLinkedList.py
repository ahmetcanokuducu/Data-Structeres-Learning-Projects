...
class ClinkendList:
    ...
    def insert_Before(self, data, newdata): 
        ...
        if (temp.data == data):
            ...
            while(temp.next.data != data and temp.next != self.start):
                temp = temp.next
            if(temp.next.data != data and temp.next == self):
                print(data, " Not Found")
            else:
                NewNode = Node(newdata)
                NewNode.next = temp.next
    ...
    def del_after(self, data):
        ...
        if (temp.data != data and temp.next == self.start):
            ...
            print(data, " is the Last node... the next node is Frist node")
            ch = input("Do yout to delete the frist node (y | Y): ")
            if (ch == 'y' or ch=='Y'):
                self.del_first()
                return
        else:
    ...
    def del_before(self, data):
        ...
        if((temp.data == data and self.start == self.end) or temp.next.data == data):
            ...
            while(temp.data != data and temp == self.start):
                temp = temp.next
            if (temp.data != data and temp.next == self.start):
                print(data, " data Not found")
            else:
...
def menu():
    ...
    print("2. Inseert at the Beginnening")
    print("3. Inseert at the End")
    print("4. Inseert After a given Data")
    print("5. Inseert before a given Data")
    print("6. Delete the Frist node")
